...
unsigned char* makeIsLessCode(struct LexemInfo** lastLexemInfoInTable, unsigned char* currBytePtr, unsigned char generatorMode) {
	unsigned char multitokenSize = detectMultiToken(*lastLexemInfoInTable, MULTI_TOKEN_LESS);
	if (multitokenSize) {
		if (generatorMode == MACHINE_X86_WIN32_CODER_MODE) {
			...
			const unsigned char code__setl_al[] = { 0x0F, 0x9C, 0xC0 };
			...
			currBytePtr = outBytes2Code(currBytePtr, (unsigned char*)code__setl_al, 3);
			...
			currBytePtr += sprintf((char*)currBytePtr, "    setl al\r\n");
